...
bStatus_t RGBLED_AddService(void)
{
  ...
  status = GATTServApp_RegisterService(rgbLedAttrTbl, GATT_NUM_ATTRS(rgbLedAttrTbl), GATT_MAX_ENCRYPT_KEY_SIZE, &rgbLedCBs);
  Log_info1("Registered service, %d attributes", (IArg)GATT_NUM_ATTRS(rgbLedAttrTbl));
    ...
    case RGB_LED_PARAM_COLOR:
      if (len == RGB_LED_COLOR_VALUE_LEN)
      {
        if (memcmp(rgbLedColor, value, RGB_LED_COLOR_VALUE_LEN))
        {
          ...
          Log_info2("SetParameter: %s len: %d", (IArg)"RGBLED", (IArg)len);
        }
      }
      else {
        ret = bleInvalidRange;
        Log_error2("RGBLED length not equal to %d, val: %d", RGB_LED_COLOR_VALUE_LEN, len);
      }
      break;

    default:
      ret = INVALIDPARAMETER;
      Log_error1("RGBLED SetParameter: Parameter #%d not valid.", (IArg)param);
      break;
  }
        ...
        if (status == SUCCESS)
        {
          ...
          if (memcmp(pCurValue, pValue, RGB_LED_COLOR_VALUE_LEN))
          {
            if (rgbLedServiceColorChangeCB)
                rgbLedServiceColorChangeCB(connHandle, RGB_LED_SERV_UUID, method, pValue, len);
            memcpy(pCurValue, pValue, RGB_LED_COLOR_VALUE_LEN); // Save the value

            Log_info2("RGBLED WriteAttrCB (CCCD): connHandle: %d %s",
                          (IArg)connHandle,
